    ...
    router.post('/:noteID/vote', async (req, res) => {
        ...
        try {
            if (!on) {
                ...
                if (!action) {
                    let commentVote = await addCommentVote({ voteType, feedbackDocID, noteDocID, voterStudentDocID })
                    let upvoteCount = commentVote["feedbackDocID"]["upvoteCount"]
                    ...
                    //         voterStudentDocID: voterStudentDocID,
                    //         feedback: true
                    //     }).sendVoteNotification(commentVote)
                    // })() : false
