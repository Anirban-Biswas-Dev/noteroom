...
let savedNotes = manageStorage.getContent('savedNotes').map(item => item.noteID)
//* Observer's object
const observers = {
	observer: function() {
		...
		const _observer = new IntersectionObserver(async entries => {
			...
			if(lastNote.isIntersecting) {
				...
				if(noteList.length != 0) {
					...
					_observer.observe(document.querySelector('.feed-note-card:last-child'))
				} else {
...
if ((navigate.type === 'navigate') || (navigate.type == 'reload')) {
	...
	Datas.forEach(CardStr => {
		...
		data.map(note => {
			...
			} // The note will be added which was uploaded and not being reloaded
			...
			}  // The save note will be added, i. remove the no-saved-notes msg
			else if(note.isNoti) {
				addNoti(note)
			} // Notis. will be added
		})
